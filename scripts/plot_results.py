...
plt.rcParams.update({"font.family": "DejaVu Sans", "text.color": "#e8eef4",
                     "axes.labelcolor": "#abbccd", "xtick.color": "#abbccd",
                     "ytick.color": "#abbccd", "font.size": 11})
fig, axes = plt.subplots(1, 2, figsize=(13, 4.7), facecolor="#0b1117")
...
colors = ["#cdfb75", "#f4c386", "#f1a477", "#8191f5"]
for ax in axes:
    ax.set_facecolor("#0b1117")
    for spine in ax.spines.values():
        spine.set_visible(False)
    ax.grid(axis="y", color="#26333f", alpha=.7)
...
axes[0].set_title("Model performance on 54 test rows", color="#e8eef4", loc="left", pad=20)
for i, v in enumerate(values):
    axes[0].text(i, v+3, f"{v:.1f}%", ha="center", color="#e8eef4", fontsize=12)
...
axes[1].set_title("Distribution + missingness signals", color="#e8eef4", loc="left", pad=20)
for i, v in enumerate(counts):
    axes[1].text(i, v+.12, str(v), ha="center", color="#e8eef4", fontsize=12)
fig.text(.06, .035, "UCI Wine · seed 42 · logistic regression · PSI ≥ 0.20 & BH q ≤ 0.05, or missingness change ≥ 5 pp\nShifted and missing-data scenarios are synthetic stress tests; original test labels are retained.", color="#97aabd", fontsize=10, linespacing=1.8)
